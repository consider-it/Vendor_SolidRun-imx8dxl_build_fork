...
COMPONENTS="atf uboot mkimage seco scfw linux safsdio llc saffirmware safboot sxf1800"
mkdir -p build
dlfailed=0
...
if [ ! -f rootfs.e2.orig ] || [[ ${ROOTDIR}/${BASH_SOURCE[0]} -nt rootfs.e2.orig ]]; then
	...
	fakeroot debootstrap --variant=minbase \
		...
		--foreign \
		--include=apt-transport-https,bluez,busybox,ca-certificates,can-utils,command-not-found,curl,e2fsprogs,ethtool,fdisk,gpiod,gpsd,gpsd-tools,gpsd-clients,haveged,i2c-tools,ifupdown,iputils-ping,isc-dhcp-client,iw,libnss-resolve,initramfs-tools,libpcap0.8,libgps28,locales,nano,net-tools,ntpdate,openssh-server,psmisc,python3-gps,python3-serial,rfkill,sudo,systemd-sysv,systemd-timesyncd,tio,usbutils,wget,wpasupplicant,xterm \
		bullseye \
		stage1 \
